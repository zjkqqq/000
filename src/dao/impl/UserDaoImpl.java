...
public class UserDaoImpl implements IUserDao {//实现IUserDao接口
	//动态sql创建statement
	...
	public boolean doCreate(User user) throws Exception {
		...
		if(this.pstmt.executeUpdate() > 0){//用于执行INSERT、UPDATE或DELETE语句以及SQL DDL语句
			flag = true;
		}
		this.pstmt.close();
		return flag;
	...
	public User findByUserName(String name) throws Exception {
		...
		ResultSet rs = this.pstmt.executeQuery();//查询数据库
		if(rs.next()){
			user = new User();
